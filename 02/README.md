...
Теперь, когда структура стала проще, можно написать для нее более предсказуемые стили. Полностью замените содержимое файла `src/styles.css` на этот код:
...
* **Отображать текущий курс:** Добавьте под полями ввода текстовый элемент, который будет показывать текущий обменный курс между выбранными валютами. Например: `1 USD = 83 RUB`. Этот элемент тоже должен обновляться при изменении выбора в любом из `<select>`.
